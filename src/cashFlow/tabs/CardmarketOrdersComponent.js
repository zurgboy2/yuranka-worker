...
import {
  decodeCardDetails,
  normalizeTcgName,
} from "../../lister/detailsDecoder";

const CardmarketOrdersComponent = () => {
  ...
  const handleOrderDataChange = (field, value) => {
    setParsedOrder((prev) => ({ ...prev, [field]: value }));
  };
  ...
  const handleSummaryChange = (field, value) => {
    setParsedOrder((prev) => ({
      ...prev,
      orderSummary: { ...prev.orderSummary, [field]: value },
    }));
  };
  ...
  const removeItem = (index) => {
    setParsedOrder((prev) => ({
      ...
      orderSummary: {
        ...prev.orderSummary,
        articleCount:
          prev.orderSummary.articleCount - prev.items[index].quantity,
      },
    }));
  };
  ...
  const parseAndValidateOrder = () => {
    ...
    try {
      ...
      const shippingAddressStart = lines.findIndex(
        (line) => line === "Shipping address",
      );
      const shippingMethodStart = lines.findIndex(
        (line) => line === "Shipping Method:",
      );
      ...
      if (summaryStart === -1)
        throw new Error("Could not locate order summary");

      orderData.orderSummary = {
        articleCount: parseInt(lines[summaryStart + 2].split(" ")[0]),
        articleValue: parseFloat(
          lines[summaryStart + 4].replace(",", ".").replace(" €", ""),
        ),
        shippingCost: parseFloat(
          lines[summaryStart + 6].replace(",", ".").replace(" €", ""),
        ),
        total: parseFloat(
          lines[summaryStart + 8].replace(",", ".").replace(" €", ""),
        ),
      };
      ...
      for (let i = itemsStartIndex + 2; i < lines.length; i++) {
        ...
        if (line.includes("x\t")) {
          ...
          const normalizedTcg =
            normalizeTcgName(orderData.tcg) || orderData.tcg;
      ...
      {parsedOrder && (
        <Box sx={{ mt: 2, mb: 2 }}>
          <Alert severity="success">
            Order successfully parsed! Please verify and edit the details if
            needed:
          </Alert>
          <Paper sx={{ p: 2, mt: 2 }}>
            <Box sx={{ mb: 2 }}>
              <TextField
                ...
                onChange={(e) =>
                  handleOrderDataChange("orderNumber", e.target.value)
                }
                sx={{ mr: 2 }}
              />
              <TextField
                ...
                onChange={(e) =>
                  handleOrderDataChange("customerUsername", e.target.value)
                }
                sx={{ mr: 2 }}
              />
              <TextField
            ...
            <Box
              sx={{ pl: 2, mb: 2, display: "flex", gap: 2, flexWrap: "wrap" }}
            >
              <TextField
                type="number"
                label="Article Count"
                ...
                onChange={(e) =>
                  handleSummaryChange("articleCount", e.target.value)
                }
              />
              <TextField
                type="number"
                ...
                onChange={(e) =>
                  handleSummaryChange("articleValue", e.target.value)
                }
              />
              <TextField
                type="number"
                ...
                onChange={(e) =>
                  handleSummaryChange("shippingCost", e.target.value)
                }
              />
              <TextField
                type="number"
                      ...
                      <TableRow key={index}>
                        ...
                        <TableCell>
                          <TextField
                            value={item.rarity}
                            onChange={(e) =>
                              handleItemChange(index, "rarity", e.target.value)
                            }
                            size="small"
                            sx={{ width: 60 }}
                          />
...
export default CardmarketOrdersComponent;
